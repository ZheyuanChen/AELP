"""
Campaign C (2D) analysis -- lasy-exporter cell vs native-injector cell.

Adapted from campaign A's 2d/scripts/analyse.py (same waist-scan method,
same snapshot-timing interpolation fix), reduced to the two campaign C
cells:
  native    -> EPOCH's own gauss()/quadratic-phase deck expressions
  lasy_file -> binary pair from the lasy fork's write_to_file exporter

Interpretation differs from campaign A: there the two sides shared the
IDENTICAL closed-form source, so the residual was pure injector error
(~1e-3 %). Here the sources deliberately differ (angular-spectrum lasy
envelope vs paraxial formula), so the expected w(x) residual is the known
lasy-vs-paraxial physics difference at NA~0.265 -- of order 1 %,
amplitude-dominated (June 2025 numbers: ~1.9 % amplitude, ~0.5 % phase).
A residual far above that, or one that looks like a CEP/carrier error in
the field comparison, indicates an exporter bug, not physics.
...
def analyse_cell(name):
    ...
    # number, kept so the size of the quantisation error is visible).
...
ax.plot(x_theory * 1e6, w_theory * 1e6, "k-", lw=1.2, label="theory (paraxial)")
...
ax.set(xlabel="x (um)", ylabel="beam radius w (um)",
       title="Campaign C (2D): lasy exporter vs native injector")
...
# As in campaign A, the pass/fail metric is the cells matching EACH OTHER,
# not matching the idealised theory curve (finite-grid effects offset the
# whole chain from theory equally). Unlike campaign A the expected
# cell-vs-cell residual is NOT the ~1e-3 % injector floor: the two cells
# use genuinely different source physics (angular-spectrum vs paraxial),
# so the plot below should show a smooth ~O(1 %) systematic, mostly from
# the amplitude profile difference at the boundary.
...
ax.set(xlabel="x (um)", ylabel="relative w(x) difference vs native (%)",
       title="Campaign C (2D): exporter+physics residual "
...
summary = f"""Campaign C (2D) -- lasy-fork exporter vs native injector
=========================================================
Beam: lambda0={P.LAMBDA0*1e6:.3f}um w0={P.W0*1e6:.3f}um NA~{P.NA:.4f}
(campaign A's beam; 1D-transverse slab, HALF Gouy phase).
...
the exact ideal time (see analyse_cell docstring). Nearest-snapshot
...
June 2025: ~1.9 % amplitude / ~0.5 % phase at the boundary). Values far
above that, or a field comparison showing a global sign flip / constant
carrier offset, indicate an exporter convention bug -- check the pytest
suite (Tests and Validation/lasy_epoch_export/) and the local
crosscheck_legacy_pipeline.py before suspecting EPOCH.
"""
