...
def animate_triptych(ref_da, var_da, diff_da, times_fs, out_path,
                     ref_label="Analytical", var_label="Numerical",
                     x_name="X_Grid_mid", y_name="Y_Grid_mid"):
    """
    Animated version of save_triptych -- same 3-panel layout (reference,
    variant, difference), stepping through all snapshots instead of one.
    Added alongside (not replacing) the static save_triptych: the module
    docstring's "No animations (head-less)" note described the ORIGINAL
    scope of this library; animations were added later on request and
    are equally head-less (Agg backend, saved to file, never displayed).
    """
    xu = _coord_um(ref_da, x_name)
    yu = _coord_um(ref_da, y_name)
    ext = [xu.min(), xu.max(), yu.min(), yu.max()]

    a_all = orient_xy(ref_da).values
    b_all = orient_xy(var_da).values
    d_all = orient_xy(diff_da).values
    vmax_field = max(np.abs(a_all).max(), np.abs(b_all).max())
    vmax_diff = np.abs(d_all).max() if np.abs(d_all).max() > 0 else 1e-30

    fig, ax = plt.subplots(1, 3, figsize=(18, 4.6))
    im0 = ax[0].imshow(a_all[0], origin="lower", aspect="auto", extent=ext,
                       cmap="RdBu_r", vmin=-vmax_field, vmax=vmax_field)
    ax[0].set(xlabel="x (µm)", ylabel="y (µm)"); fig.colorbar(im0, ax=ax[0])
    im1 = ax[1].imshow(b_all[0], origin="lower", aspect="auto", extent=ext,
                       cmap="RdBu_r", vmin=-vmax_field, vmax=vmax_field)
    ax[1].set(xlabel="x (µm)", ylabel="y (µm)"); fig.colorbar(im1, ax=ax[1])
    im2 = ax[2].imshow(d_all[0], origin="lower", aspect="auto", extent=ext,
                       cmap="RdBu_r", vmin=-vmax_diff, vmax=vmax_diff)
    ax[2].set(xlabel="x (µm)", ylabel="y (µm)"); fig.colorbar(im2, ax=ax[2])
    t0_title = [ax[0].set_title(f"{ref_label} [$a_0$]  |  t = {times_fs[0]:.1f} fs"),
               ax[1].set_title(f"{var_label} [$a_0$]  |  t = {times_fs[0]:.1f} fs"),
               ax[2].set_title(f"Difference ({var_label} - {ref_label}) [$a_0$]  |  "
                               f"t = {times_fs[0]:.1f} fs")]
    fig.tight_layout()

    def update(i):
        im0.set_array(a_all[i]); im1.set_array(b_all[i]); im2.set_array(d_all[i])
        t0_title[0].set_text(f"{ref_label} [$a_0$]  |  t = {times_fs[i]:.1f} fs")
        t0_title[1].set_text(f"{var_label} [$a_0$]  |  t = {times_fs[i]:.1f} fs")
        t0_title[2].set_text(f"Difference ({var_label} - {ref_label}) [$a_0$]  |  "
                             f"t = {times_fs[i]:.1f} fs")
        return im0, im1, im2, *t0_title

    anim = FuncAnimation(fig, update, frames=len(times_fs), blit=False, interval=150)
    try:
        anim.save(out_path, writer="ffmpeg", dpi=120)
    except Exception:
        out_path = os.path.splitext(out_path)[0] + ".gif"
        anim.save(out_path, writer="pillow", dpi=100, fps=6)
    plt.close(fig)
    return out_path
